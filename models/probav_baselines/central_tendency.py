...
class MedianModel:
    """
    Wrapper class to make the central_tendency function compatible 
    with the pipeline's .predict() interface.
    """
    def __init__(self, agg_with='median'):
        self.agg_with = agg_with

    def predict(self, lr: np.ndarray) -> np.ndarray:
        """
        Takes the input tensor (T, H, W) and converts it 
        to the (image, mask) pairs required by central_tendency.
        """
        # Create a dummy mask because the central_tendency function 
        # requires (image, mask) pairs.
        dummy_mask = np.ones((lr.shape[1], lr.shape[2]), dtype=bool)
        
        # Prepare the list of (image, mask) pairs
        images_with_masks = [(frame, dummy_mask) for frame in lr]
        
        # Run the aggregation
        return central_tendency(images_with_masks, agg_with=self.agg_with)